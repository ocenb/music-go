...
func (c *Client) CheckAuth(ctx context.Context, authorizationHeader string) error {
	outCtx := metadata.NewOutgoingContext(ctx, metadata.Pairs("authorization", authorizationHeader))

	_, err := c.client.CheckAuth(outCtx, &emptypb.Empty{})
	if err != nil {
		return fmt.Errorf("user service check auth: %w", err)
	}
	return nil
}
