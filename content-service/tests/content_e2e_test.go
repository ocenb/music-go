...
func TestHistoryEmptyE2E(t *testing.T) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, env.BaseURL+"/api/content/history?take=10", http.NoBody)
	require.NoError(t, err)
...
func TestInternalDeleteUserE2E(t *testing.T) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodDelete, env.BaseURL+"/api/content/internal/users/999", http.NoBody)
	require.NoError(t, err)
